...
const icons = [
    ...
    {
        icon: <SiAntdesign size={59}  className='hover:text-[#F73F50]' />,
        name: "Ant Design",
        bg: "#F73F50"
    }
]
...
function About() {
  return (
    ...
    <div className='flex flex-col md:flex-row flex-wrap justify-between px-6'>
        <div className='w-full md:w-[37%] md:h-[500px]'>
            <img src={pic} alt="" />
        </div>
        ...
        <h3 className="text-2xl lg:text-[30px] font-semibold text-center line-clamp-2 mb-4">
            ...
            <div className='flex flex-wrap items-center justify-center gap-8 mt-10  socials-container'>
                {
                    icons.map((val, ind) => (
                        <span style={{"--accent-color":val.bg}} data-social={val.name} key={ind} className='relative text-gray-300 cursor-pointer'>{val.icon}</span>
                    ))
                }
            </div>
